...
import type { NotationStyle } from "../../types/music";
export type { NotationStyle }; // re-export so MusicComposer's import stays unchanged
...
interface ComposerFormProps {
  ...
  onGenreChange: (v: Genre) => void;
  onStructureChange: (v: StructureType) => void;
  onNotationChange: (v: NotationStyle) => void;
  onGenerate: () => void;
}
  ...
  return (
    <div className="flex flex-col gap-5 h-full">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          >
            ...
            <option value="AB">AB — Binary</option>
            <option value="AABB">AABB — Rep. binary</option>
            <option value="ABACA">ABACA — Rondo</option>
            <option value="through">Through-composed</option>
          </Select>
        </div>
        <div>
          ...
          <Select
            value={notation}
            onChange={(e) => onNotationChange(e.target.value as NotationStyle)}
          >
            <option value="lead-sheet">Lead Sheet</option>
            <option value="classical">Classical (Grand Staff)</option>
          </Select>
        </div>
      </div>
      ...
      <div className="rounded border border-stone-700 bg-stone-900 px-3 py-2.5 text-xs text-stone-400 leading-relaxed">
        {notation === "lead-sheet" && (
          <>
            <span className="text-amber-400 font-semibold">Lead Sheet: </span>
            Right hand melody with chord symbols above. Ideal for improvisation,
            jazz, folk, and singer-songwriters.
          </>
        )}
        {notation === "classical" && (
          <>
            <span className="text-amber-400 font-semibold">
              Classical (Grand Staff):{" "}
            </span>
            Both treble and bass clef staves with chord symbols retained. Full
            piano arrangement with left-hand accompaniment.
          </>
        )}
      </div>
