...
// by the key signature and should NOT be drawn on individual notes.

const KEY_SIG_ACCIDENTALS: Record<string, string[]> = {
  C: [],
...
 * Returns true if the accidental on this pitch is already implied by
the

 * key signature — i.e. it should NOT be drawn as an individual
accidental.

 */

function isAccidentalInKeySig(pitch: string, keySig: string): boolean {
  ...
  const covered = KEY_SIG_ACCIDENTALS[keySig] ?? [];

  return covered.includes(letter);
}
...
function splitIntoBars(
  ...
  return bars.map((bar, i) => {
    ...
    if (Math.abs(total - ticksPerBar) > 2) {
      console.warn(`Bar ${i}: ${total} ticks (expected ${ticksPerBar}),
auto-correcting`);
...
function makeNote(
  ...
  if (isDotted) Dot.buildAndAttach([sn], { all: true }); // Only add an accidental modifier if it is NOT already covered by the
  // key signature — otherwise the key sig at the start of each line
  // already tells the reader about it.

  const acc = getAccidental(safePitch);

  if (acc && !isAccidentalInKeySig(safePitch, keySig)) {
