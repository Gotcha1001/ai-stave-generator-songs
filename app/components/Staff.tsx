...
interface StaffProps {
  ...
  left: NoteType[];
  currentBeat?: number;
  timeSig?: string; // NEW: "4/4", "3/4", "6/8", "2/4" — defaults to "4/4"
}
...
// ── Time signature → VexFlow voice params + internal tick budget ────────────
function getVoiceAndTickParams(timeSig: string = "4/4") {
  switch (timeSig) {
    case "3/4":
      return { numBeats: 3, beatValue: 4, ticksPerBar: 300 };
    case "6/8":
      return { numBeats: 6, beatValue: 8, ticksPerBar: 300 }; // 6 eighths = 3 quarters
    case "2/4":
...
function clampPitch(pitch: string, clef: "treble" | "bass"): string {
  ...
  let octave = parseInt(m[3]);
  if (clef === "treble") {
    octave = Math.min(Math.max(octave, 4), 6);
  } else {
    octave = Math.min(Math.max(octave, 2), 4);
  }
  return `${note}${acc}${octave}`;
}
...
function splitIntoBars(
  ...
  // Safety: ensure exact tick count
  return bars.map((bar, i) => {
    const total = bar.reduce((s, n) => s + (DUR_TO_TICKS[n.duration] ?? 0), 0);
    if (Math.abs(total - ticksPerBar) > 1) {
      console.warn(`Bar ${i} has ${total} ticks (expected ${ticksPerBar})`);
...
export default function Staff({
  ...
  // ── Playhead highlight overlay (canvas) ────────────────────────────────────
  useEffect(() => {
    const container = ref.current;
    const canvas = canvasRef.current;
    if (!canvas || !container) return;
    ...
    const beatsPerBar = ticksPerBar / 100; // assuming q=100 ticks
