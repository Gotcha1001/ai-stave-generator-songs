...
/**
 * Build the set of note names (e.g. "F#", "C#", "Bb") whose accidentals are
 * already declared in the key signature and must NOT be rendered inline.
 * These match exactly what parsePitch() returns in `parsed.note`.
 */
function buildKeySigNotes(ks: { sharps: number; flats: number }): Set<string> {
  const set = new Set<string>();
  for (let n = 0; n < ks.sharps; n++) set.add(SHARP_ORDER[n] + "#");
...
export default function ScoreRenderer({
  ...
  useEffect(() => {
    ...
    const ks = KEY_SIGNATURES[piece.key];
    // Build once — shared by every drawBarNotes call for this piece.
    const keySigNotes = ks ? buildKeySigNotes(ks) : new Set<string>();
    ...
    for (let row = 0; row < numRows; row++) {
      ...
      // ── Key + time signatures (row 0 only) ────────────────────────────────
      let ksSigWidth = 0;
      let headerEndX = MARGIN_LEFT;
      ...
      if (row === 0) {
        if (ks) {
          ...
          // Bass staff must also show the key signature
          if (isGrandStaff)
            drawKeySignature(ctx, MARGIN_LEFT + 10, bassTop, ks, SL);
        }
      ...
      // ── Row barline (rows after the first) ────────────────────────────────
      if (row > 0) {
        ctx.strokeStyle = "#2a2015";
        ctx.lineWidth = 1;
        ctx.beginPath();
      ...
      for (let b = 0; b < rowBars.length; b++) {
        ...
        drawBarNotes(
          ...
          "treble",
          SL,
          keySigNotes, // ← key-sig awareness: no spurious accidentals
        );

        // Notes — bass staff (this was the broken one)
        if (isGrandStaff && bar.leftNotes?.length) {
          drawBarNotes(
            ctx,
            bar.leftNotes,
            ...
            "bass",
            SL,
            keySigNotes, // ← same keySigNotes, suppresses inline ♯/♭ on key-sig notes
          );
        }
      }
...
function drawBarNotes(
  ...
  clef: Clef,
  sl: number,
  keySigNotes: Set<string>, // notes already covered by the key signature
) {
  if (!notes.length) return;
  ...
  for (let idx = 0; idx < notes.length; idx++) {
    ...
    // ── Accidental: only show if NOT already in the key signature ───────────
    // parsed.note is exactly "F#", "Bb", "C#" etc. — same format as keySigNotes.
    // If the note has no accidental character at all, accidental is null.
    // If it has one but the key signature already covers it, also null.
    const hasAccChar = parsed.note.includes("#") || parsed.note.includes("b");
    const accidental =
      hasAccChar && !keySigNotes.has(parsed.note)
        ? parsed.note.includes("#")
          ? "♯"
          : "♭"
        : null;
