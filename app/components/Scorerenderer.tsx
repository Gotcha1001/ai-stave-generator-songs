...
// /**
//  * Build set of accidentals already covered by the key signature
//  */
// function buildKeySigNotes(ks: { sharps: number; flats: number }): Set<string> {
//   const set = new Set<string>();
//   for (let n = 0; n < ks.sharps; n++) set.add(SHARP_ORDER[n] + "#");
...
// /**
//  * Decide whether to draw an accidental symbol.
//  * Since we now store full accidentals (F#4, Bb3, etc.), we only show the symbol
//  * if it's NOT already implied by the key signature.
//  */
...
//   const hasAcc = parsed.note.includes("#") || parsed.note.includes("b");
//   if (!hasAcc) return null;

//   // Suppress accidental if key signature already covers it
//   if (keySigNotes.has(parsed.note)) return null;

//   return parsed.note.includes("#") ? "♯" : "♭";
// }
...
//     const ks = KEY_SIGNATURES[piece.key] || { sharps: 0, flats: 0 };
//     const keySigNotes = buildKeySigNotes(ks);
...
//       // Key + Time Signature (first row only)
//       let ksSigWidth = 0;
//       let headerEndX = MARGIN_LEFT;

//       if (row === 0) {
//         if (ks.sharps > 0 || ks.flats > 0) {
//           ksSigWidth = drawKeySignature(
//             ctx,
//             MARGIN_LEFT + 10,
//             staffTop,
...
//       // Row barline for subsequent rows
//       if (row > 0) {
//         ctx.strokeStyle = "#2a2015";
//         ctx.lineWidth = 1;
//         ctx.beginPath();
...
//   const noteXs = computeNoteXPositions(notes, beatGrid, timeSig);
...
//   // Draw noteheads, stems, beams, etc.
//   const rw = sl * 0.62;
//   const rh = sl * 0.46;
...
//     const accidental = shouldShowAccidental(note.pitch, keySigNotes);
...
//   // Beams (unchanged from your original)
//   const rwBeam = sl * 0.62;
//   const groups = new Map<number, number[]>();
//   for (let idx = 0; idx < notes.length; idx++) {
//     const g = beamGroup[idx];
...
//     const firstNoteY = getNoteY(first);
//     const stemUp = firstNoteY > staffTop + 2 * sl;
//     const groupNoteYs = indices.map((idx: number) => getNoteY(idx));
//     const rawStemEnds = groupNoteYs.map((ny: number) =>
//       stemUp ? ny - sl * 3.5 : ny + sl * 3.5,
//     );
...
//       if (deficit > 0) {
//         const shift = stemUp ? -deficit : deficit;
//         if (stemUp ? shift < beamShift : shift > beamShift) beamShift = shift;
//       }
//     }
//     beamY1 += beamShift;
//     beamY2 += beamShift;
...
//     // Secondary beam for semiquavers
//     const beamThickness = sl * 0.55;
//     const beamGap = beamThickness + 2;
//     const secondBeamOffset = stemUp ? beamGap : -beamGap;
//     const stubW = sl * 1.2;
...
const MARGIN_LEFT = 90;
const MARGIN_RIGHT = 40;
const MARGIN_TOP = 118; // Increased to prevent subtitle/chord overlap
const STAFF_GAP = 80;
const CLEF_WIDTH = 22;

// ── Page-break constants ────────────────────────────────────────────────────
// A4 at 96 dpi ≈ 1123 px. Rows whose bottom would cross a page boundary are
// pushed to the top of the next page instead, so nothing ever gets clipped.
const PAGE_HEIGHT = 1123;
const PAGE_MARGIN_BOTTOM = 60; // extra breathing room before the page edge

/**
 * Pre-compute the absolute Y position of each staff row, inserting page-break
 * gaps whenever a row would otherwise straddle a PDF page boundary.
 */
function computeRowYPositions(
  numRows: number,
  rowHeight: number,
  marginTop: number,
): { rowYs: number[]; totalHeight: number } {
  const rowYs: number[] = [];
  let cursorY = marginTop;
  let pageStartY = 0; // top of the current virtual page

  for (let row = 0; row < numRows; row++) {
    if (row > 0) {
      const posOnPage = cursorY - pageStartY;
      // If this row's bottom would exceed the safe area of the current page,
      // jump to the top of the next page.
      if (posOnPage + rowHeight > PAGE_HEIGHT - PAGE_MARGIN_BOTTOM) {
        pageStartY += PAGE_HEIGHT;
        cursorY = pageStartY + marginTop;
      }
    }
    rowYs.push(cursorY);
    cursorY += rowHeight;
  ...
  return { rowYs, totalHeight: cursorY + 80 };
}
...
export default function ScoreRenderer({
  ...
  useEffect(() => {
    ...
    // ── Page-aware row positions ──────────────────────────────────────────
    const { rowYs, totalHeight: canvasH } = computeRowYPositions(
      numRows,
      ROW_HEIGHT,
      MARGIN_TOP,
    );
    ...
    ctx.textAlign = "center";
    ctx.fillText(piece.title, canvasW / 2, 48);

    // Subtitle (key, time sig, genre, tempo)
    ctx.fillStyle = "#6b5c44";
    ...
    for (let row = 0; row < numRows; row++) {
      ...
      // ── Use page-aware Y instead of a simple linear offset ─────────────
      const staffTop = rowYs[row];
      const bassTop = staffTop + 4 * SL + STAFF_GAP;
      const staffRight = MARGIN_LEFT + rowBars.length * BAR_WIDTH;
      ...
      for (let b = 0; b < rowBars.length; b++) {
        ...
        // ==================== CHORD SYMBOL - AT BEGINNING OF BAR ====================
        if (bar.chordName || bar.chord) {
          ctx.fillStyle = "#111111"; // Clean black
          ctx.font = 'italic 12.5px "Georgia", serif'; // Nice size
          ctx.textAlign = "left";

          const chordText = (bar.chordName || bar.chord || "").trim();
          const chordY = staffTop - 28; // High and clean

          ctx.fillText(chordText, barStartX + 12, chordY);
        }
        // ===========================================================================
  ...
  // Playhead overlay — mirrors the page-aware Y positions
  useEffect(() => {
    const score = scoreCanvasRef.current;
    const overlay = overlayCanvasRef.current;
    if (!score || !overlay) return;
    ...
    // ── Same page-aware positions as the score canvas ───────────────────
    const { rowYs } = computeRowYPositions(numRows, ROW_HEIGHT, MARGIN_TOP);
...
function drawBarNotes(
  ...
  // Beams (unchanged - kept for completeness)
  const rwBeam = sl * 0.62;
  const groups = new Map<number, number[]>();
  for (let idx = 0; idx < notes.length; idx++) {
    const g = beamGroup[idx];
